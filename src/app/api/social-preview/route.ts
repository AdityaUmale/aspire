...
export async function GET(request: NextRequest) {
  const rawRequestedUrl = request.nextUrl.searchParams.get('url')?.trim();
  const facebookPlugin = rawRequestedUrl && /facebook\.com/i.test(rawRequestedUrl) ? getFacebookPluginDetails(rawRequestedUrl) : null;
  const requestedUrl =
    rawRequestedUrl && /facebook\.com/i.test(rawRequestedUrl)
      ? normalizeFacebookInput(rawRequestedUrl)
      : rawRequestedUrl;
  ...
  if (!['http:', 'https:'].includes(parsedUrl.protocol) || !isAllowedHost(parsedUrl.hostname)) {
    return NextResponse.json({ success: false, message: 'Unsupported host' }, { status: 400 });
  }
    ...
    return NextResponse.json(
      {
        success: true,
        data: {
          canonicalUrl: facebookPlugin.targetUrl,
          title: 'Facebook post',
          description: '',
          embedKind: facebookPlugin.embedKind,
          embedUrl: facebookPlugin.embedUrl,
          finalUrl: facebookPlugin.targetUrl,
          image: '',
  ...
  try {
    ...
    if (!isAllowedHost(finalParsedUrl.hostname)) {
      return NextResponse.json({ success: false, message: 'Redirected to an unsupported host' }, { status: 400 });
    }
    ...
    return NextResponse.json(
      { success: false, message: 'Failed to fetch preview metadata' },
      {
        status: 500,
        headers: {
          'Cache-Control': 'no-store, max-age=0',
